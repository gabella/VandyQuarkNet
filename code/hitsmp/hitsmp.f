...
c
c Invoke program with command line "hits n file" where file is the name
c of the file from the Purdue program (up to 120 char) and n is an integer
c 0 for no output file, 1 for muon speed cvs file, 2 for shower data file, 
c and 3 for both output files.
c     
      include 'com_mp.inc'
      ...
      data ictm/13/   ! Decimal 13 as char is CR (carriage return)
      data fullf/
...
c
      ...
      If (numarg.lt.1 .or.numarg.gt.2) Then
        write(6,*)'Correct usage is integer then file name'
        Call exit(2)
      Endif
      Call getarg(1,bufin)
 ...
 10   read(2,1000,err=900,end=901)bufin
      nlines=nlines+1
c      write(6,*)'*** ', nlines, ' ', lnblnk(bufin), ' ', 'X', bufin, 'X'  ! WEG added for debugging.
...
c  SA  35  followed by 1 line
 ...
 9011 format('Data taken by unit ',a12,' in file'/a120)
